...
from djangoredis import pool
from djangoredis.exceptions import CompressorError, ConnectionInterrupted
from djangoredis.util import CacheKey
...
class DefaultClient:
    def __init__(self, server, params: Dict[str, Any], backend: BaseCache) -> None:
        ...
        self.reverse_key = get_key_func(
            ...
            or "djangoredis.util.default_reverse_key"
        )
        ...
        serializer_path = self._options.get(
            "SERIALIZER", "djangoredis.serializers.pickle.PickleSerializer"
        )
        serializer_cls = import_string(serializer_path)

        compressor_path = self._options.get(
            "COMPRESSOR", "djangoredis.compressors.identity.IdentityCompressor"
        )
    ...
    def _has_compression_enabled(self) -> bool:
        return (
            ...
            != "djangoredis.compressors.identity.IdentityCompressor"
        )
