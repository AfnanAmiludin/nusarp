...
from .config import DjangoHueySettingsReader
...
def db_task(*args, **kwargs):
    def decorator(function):
        if 'queue' not in kwargs:
            kwargs.update(queue=function.__module__.split('.')[0] or None, )
        queue = kwargs.get('queue')
        then = task(*args, **kwargs)(get_close_db_for_queue(queue)(function))
        then.call_local = function
        setattr(then, 'queue', queue or function.__module__.split('.')[0] or None)
        return then
...
def db_periodic_task(*args, **kwargs):
    def decorator(function):
        if 'queue' not in kwargs:
            kwargs.update(queue=function.__module__.split('.')[0] or None, )
        queue = kwargs.get('queue')
        then = periodic_task(*args, **kwargs)(get_close_db_for_queue(queue)(function))
        then.call_local = function
        setattr(then, 'queue', queue or function.__module__.split('.')[0] or None)
        return then
