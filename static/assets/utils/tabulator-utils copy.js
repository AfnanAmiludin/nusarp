...
AppUtils.TableUtils = {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            const results = response.map(item => ({
              field: item.field,
              title: searchableColumns.find(col => col.field === item.field)?.title || item.field,
              value: item.value
            }));
            resolve(results);
          },
  ...
  initializeSearchableTable: function(tableSelector, columnDefs, options = {}) {
    ...
    function applyFilters() {
      ...
      // Update table with new filter parameters
      table.setData({
